...
def view_orders(orders):
    if not orders:
        ...
        for order in orders:
            print(f"- Order ID: {order['order_id']}, Customer: {order['customer_name']}, Dish: {order['dish']}, Status: {order['status']}")
...
def edit_ingredient(ingredients):
    ...
    for ingredient in ingredients:
        if ingredient['ingredient_id'] == ingredient_id:
            ...
            ingredient['quantity'] = int(input("Enter new ingredient quantity: "))
            save_ingredients("ingredients.txt", ingredients)
...
def view_ingredients(ingredients):
    if not ingredients:
        ...
        for ingredient in ingredients:
            print(f"- ID: {ingredient['ingredient_id']}, Name: {ingredient['name']}, Quantity: {ingredient['quantity']}")
...
# Validate login
def validate__login(chef_username, chef_password):
    with open("user.txt", "r") as file:
        for line in file:
            lst = line.strip().split(",")
...
while True:
    ...
    if choice == '1':
        ...
        if logged_in_user:
            main_chef(logged_in_user)  # Proceed to main management system if login is successful
    elif choice == '3':
        print("Exiting program...")
        break
    else:
